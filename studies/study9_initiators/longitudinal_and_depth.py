"""T4 - Longitudinal: does leader-initiation share rise with week (link to Study 5 consolidation)?
T5 - Depth by initiator type: do leader-initiated events differ in depth from member-initiated ones?
"""
import os
...
print("=" * 90, "\nT4 - Leader-initiation share vs week (per-meeting), Kendall tau\n" + "=" * 90)
long_rows = []
for defn in DEFS:
    sub = D.dropna(subset=[defn]).copy()
    sub["is_leader"] = (sub[defn] == sub.leader).astype(float)
    per_mtg = sub.groupby(["team", "mid", "week"]).is_leader.mean().reset_index()
    for team, g in per_mtg.groupby("team"):
        if len(g) >= 6:
            tau, p = kendalltau(g.week, g.is_leader)
        else:
            tau, p = np.nan, np.nan
    ...
    tau, p = kendalltau(per_mtg.week, per_mtg.is_leader)
...
LONG.to_csv(f"{BASE}/data/leader_share_vs_week.csv", index=False)

print("\n" + "=" * 90, "\nT5 - Event depth by initiator type (leader vs member)\n" + "=" * 90)
depth_rows = []
for defn in DEFS:
    ...
    for team, g in sub.groupby("team"):
        a = g[g.init_type == "leader"].depth; b = g[g.init_type == "member"].depth
        if len(a) >= 5 and len(b) >= 5:
            U, p = mannwhitneyu(a, b)
        else:
            U, p = np.nan, np.nan
        depth_rows.append(dict(definition=defn, team=team, level="event", n_leader=len(a), n_member=len(b),
                                mean_leader=a.mean(), mean_member=b.mean(), test="Mann-Whitney U", stat=U, p=p))
        print(f"{defn:14s} {team} (event-level): leader depth={a.mean():.2f} (n={len(a)}) vs "
    ...
    if len(per_mtg) >= 8:
        stat, p = wilcoxon(per_mtg["leader"], per_mtg["member"])
    else:
        stat, p = np.nan, np.nan
    depth_rows.append(dict(definition=defn, team="pooled", level="meeting", n_leader=len(per_mtg), n_member=len(per_mtg),
                            mean_leader=per_mtg["leader"].mean() if len(per_mtg) else np.nan,
                            mean_member=per_mtg["member"].mean() if len(per_mtg) else np.nan,
                            test="paired Wilcoxon", stat=stat, p=p))
    print(f"{defn:14s} pooled (meeting-level, n={len(per_mtg)}): leader mean depth={per_mtg['leader'].mean():.2f} "
...
print("\nwrote leader_share_vs_week.csv, depth_by_initiator_type.csv")
