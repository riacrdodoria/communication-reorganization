...
g1 = skew(E); k = kurtosis(E, fisher=False); n = len(E)
...
# ============================================================ Layout: 1 row x 3 cols, ~7.0 x 2.8 in
...
axC.text(0, obs + .012, f"{obs:.2f}", ha="center", fontsize=7, fontweight="bold")
axC.set_xticks(x); axC.set_xticklabels(["observed", "AR(1)\nnull", "VAR(1)\nnull"], fontsize=6.8)
axC.set_ylabel("SD of pre→post\nentropy change", fontsize=7.6); axC.set_ylim(0, .48)
...
fig.suptitle("Figure 1 — Computational structure of communication reorganization", fontsize=10.5,
             fontweight="bold", x=.01, ha="left", y=1.06)
ps.caption(fig, "Figure 1. Computational structure of communication reorganization (Study 1). (A) "
    ...
    "coefficient (BC = 0.18) falls well below the bimodality threshold (0.555) - a single graded continuum. "
    ...
    "by ~2.1x, establishing that reorganization is supra-autoregressive.", y=-0.16, fontsize=6.4)
